...
if($arParams["IBLOCK_CATOGORIES"]<=0)
{
...
if(!isset($arParams["EXT_FILTER"]) || !is_array($arParams["EXT_FILTER"]))
    $arParams["EXT_FILTER"] = array();
...
if(strlen($arParams["BLOG_URL"]) > 0)
{
	...
	elseif (IntVal($_GET["hide_id"]) > 0 && $arResult["IDEA_MODERATOR"]) //Message hide
	{
		if($arResult["BLOG"] = CBlog::GetByUrl($arParams["BLOG_URL"], $arParams["GROUP_ID"]))
		{
			if($_GET["success"] == "Y") 
				...
				if (check_bitrix_sessid())
				{
					...
					if($arResult["PostPerm"]>=BLOG_PERMS_MODERATE)
					{
						if(CBlogPost::GetByID($hide_id))
						{
							...
							{
								BXClearCache(True, "/".SITE_ID."/blog/".$arResult["BLOG"]["ID"]."/first_page/");
								BXClearCache(True, "/".SITE_ID."/blog/".$arResult["BLOG"]["ID"]."/pages/");
								BXClearCache(True, "/".SITE_ID."/blog/".$arResult["BLOG"]["ID"]."/post/".$hide_id."/");
                                                                BXClearCache(True, '/'.SITE_ID.'/idea/statistic_list/');
                                                                //RSS
                                                                BXClearCache(True, "/".SITE_ID."/idea/".$arResult["BLOG"]["ID"]."/rss_list");
                                                                
								LocalRedirect($APPLICATION->GetCurPageParam("hide_id=".$hide_id."&success=Y", Array("del_id", "sessid", "success", "hide_id", "show_id")));
							}
							else
	...
	elseif (IntVal($_GET["show_id"]) > 0 && $arResult["IDEA_MODERATOR"]) //Message show
	{
		if($arResult["BLOG"] = CBlog::GetByUrl($arParams["BLOG_URL"], $arParams["GROUP_ID"]))
		{
			if($_GET["success"] == "Y") 
	...
	$cache = new CPHPCache;
	$cache_id = "blog_blog_message_".serialize($arParams)."_".CDBResult::NavStringForCache($arParams["MESSAGE_COUNT"])."_".$strUserGroups."_".$arResult["PostPerm"].intval($arResult["IDEA_MODERATOR"]);
        if(!empty($arResult["BLOG"]))
	{
