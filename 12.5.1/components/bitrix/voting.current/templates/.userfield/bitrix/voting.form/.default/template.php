...
<form action="<?=str_replace(array("view_form=Y", "view_form"), "", POST_FORM_ACTION_URI)?>" method="post" class="vote-form" name="<?=$form?>" id="<?=$form?>">
	...
	<?=bitrix_sessid_post()?>
	<ol class="bx-vote-question-list" id="vote-<?=$uid?>"<?if($arQuestion["REQUIRED"]=="Y"){?> class="bx-vote-question-required"<?}?>>
		...
		<li id="question<?=$arQuestion["ID"]?>" >
			...
			switch ($arAnswer["FIELD_TYPE"]):
				...
				?><span class="bx-vote-block-input-wrap bx-vote-block-radio-wrap"><?
					?><span><input type="radio" name="vote_radio_<?=$arAnswer["QUESTION_ID"]?>" <?
						?>id="vote_radio_<?=$arAnswer["QUESTION_ID"]?>_<?=$arAnswer["ID"]?>" <?
						?>value="<?=$arAnswer["ID"]?>" <?=$arAnswer["~FIELD_PARAM"]?> /></span><?
				...
				case 1://checkbox
					?><span class="bx-vote-block-input-wrap bx-vote-block-checbox-wrap"><?
						?><span><input <?=$value?> type="checkbox" name="vote_checkbox_<?=$arAnswer["QUESTION_ID"]?>[]" value="<?=$arAnswer["ID"]?>" <?
							?> id="vote_checkbox_<?=$arAnswer["QUESTION_ID"]?>_<?=$arAnswer["ID"]?>" <?=$arAnswer["~FIELD_PARAM"]?> /></span><?
			...
			endswitch;?>
					...
					<td>
						...
						<span class="bx-vote-data-percent"></span>
					</td>
				</tr>
				<?endforeach;?>
			</table>
