...
$arRootCategory = array_slice($arRootCategory, 0, $arParams["CATOGORIES_CNT"]);
?>
<div id="idea-list-container">
    <div style="margin-right: 8px; padding-top:18px;"><input id="idea-field-common-show-add-form" class="idea-field-common" type="text" value="<?=GetMessage("IDEA_INPUT_TITLE_IDEA")?>" maxlength="255"></div>
    <div style="padding-top: 20px;"></div>
    <div class="status-box-l">
        <div class="status-box-r">
            <div class="status-box-m">
                <?$i=0; foreach($arRootCategory as $arCategory):?>
                <div class="status-item-categoty status-item<?if($i++==0):?>-selected<?endif;?>" id="idea-category-list-<?=ToLower($arCategory["CODE"])?>">
                    <div>
                        <div>
                            <a><?=$arCategory["NAME"]?></a>
                        </div>
