...
<script type="text/javascript">
	...
	function chartCreateYColumnSpanAdd()
	{
		...
		if (!addYColumnSpan)
		{
			...
			if (lastYcolumnBlock)
			{
				ySelect = BX.findChildren(lastYcolumnBlock, {tag: 'select', class: 'report-chart-select-col'}, true);
				if (ySelect)
				{
	...
	function chartOnClickAddYcolumn()
	{
		...
		if (block)
		{
			...
			labelNode = BX.findChildren(blockCopy, {tag: 'label', class: 'reports-title-label'}, true);
			if (labelNode)
			{
				labelNode[0].setAttribute('for', chartIncStringIndex(labelNode[0].getAttribute('for')));
				selectNode = BX.findChildren(blockCopy, {tag: 'select', class: 'report-chart-select-col'}, true);
				if (selectNode)
				{
			...
			if (blockIndex != null)
			{
				...
				if (blockIndex === 1)
				{
					prevLabelNode = BX.findChildren(block, {tag: 'label', class: 'reports-title-label'}, true);
	...
	function chartOnClickRemoveYcolumn()
	{
		...
		if (block && prevBlock)
		{
			...
			chartRemoveWithClearId('report-chart-add-ycolumn');
			chartRemoveWithClearId('report-chart-remove-ycolumn');
			labelNode = BX.findChildren(prevBlock, {tag: 'label', class: 'reports-title-label'}, true);
			if (labelNode)
			{
				selectNode = BX.findChildren(prevBlock, {tag: 'select', class: 'report-chart-select-col'}, true);
