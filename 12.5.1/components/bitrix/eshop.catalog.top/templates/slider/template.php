		...
		?>
			<div class="slide R2D2" itemscope itemtype = "http://schema.org/Product">
				<div class="slider_img">
					...
					<?if (strlen($arItem["DETAIL_PICTURE"]["SRC"])>0):?>
						<div class="photo">
							...
							<div class="badge notavailable"><?=GetMessage("CATALOG_NOT_AVAILABLE")?></div>
							<?elseif (in_array($arParams["FLAG_PROPERTY_CODE"], array("SPECIALOFFER","SPECIALOFFER","NEWPRODUCT"))):?>
				...
				<div class="info">
					...
					<p><span itemprop = "description"><?=substr(strip_tags($arItem["PREVIEW_TEXT"]), 0, 200); ?></span></p><br/>
					<!-- 445 max -->
							...
							<td>
								...
								else // if product doesn't have offers
								{
									...
									foreach($arItem["PRICES"] as $code=>$arPrice):
										if($arPrice["CAN_ACCESS"]):?>
											<?if ($numPrices>1):?><div style="font-size: 12px;"><?=$arResult["PRICES"][$code]["TITLE"];?>:</div><?endif?>
											<?if($arPrice["DISCOUNT_VALUE"] < $arPrice["VALUE"]):?>
