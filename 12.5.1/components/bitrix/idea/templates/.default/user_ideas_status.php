...
$arFilter = array(
    "AUTHOR_ID" => intval($arResult["VARIABLES"]["user_id"]),
    "IDEA_STATUS" => $StatusCode,
);

if($arResult["SET_NAV_CHAIN"] == "Y" || $arResult["SET_TITLE"])
{
    if($arResult["VARIABLES"]["user_id"] == $USER->GetID())
    {
        if($arResult["SET_NAV_CHAIN"] == "Y")
            $APPLICATION->AddChainItem(GetMessage("IDEA_USER_IDEA_LIST_MINE"), CComponentEngine::MakePathFromTemplate($arResult["PATH_TO_USER_IDEAS"], array("user_id" => $arResult["VARIABLES"]["user_id"])));
        if($arResult["SET_TITLE"] == "Y")     
            $APPLICATION->SetTitle(GetMessage("IDEA_USER_IDEA_LIST_MINE"));
    }
    elseif($arUser = $USER->GetByID($arFilter["AUTHOR_ID"])->Fetch())
    {
        ...
        if(strlen(trim($UName))==0)
                $UName = $arUser["LOGIN"];
        if($arResult["SET_NAV_CHAIN"] == "Y")
            $APPLICATION->AddChainItem(GetMessage("IDEA_USER_IDEA_LIST_USER", array("#USER_NAME#" => $UName)), CComponentEngine::MakePathFromTemplate($arResult["PATH_TO_USER_IDEAS"], array("user_id" => $arResult["VARIABLES"]["user_id"])));
        if($arResult["SET_TITLE"] == "Y")
            $APPLICATION->SetTitle(GetMessage("IDEA_USER_IDEA_LIST_USER", array("#USER_NAME#" => $UName)));
    }
}
...
<div class="idea-managment-content">
    ...
    <?$this->SetViewTarget("idea_filter", 100)?>
        <?if($arParams["DISABLE_RSS"] != "Y"):?>
            <?$APPLICATION->IncludeComponent(
                ...
                Array(
                    ...
                    "PATH_TO_RSS"		=> CComponentEngine::MakePathFromTemplate($arResult["PATH_TO_RSS_USER_IDEAS_STATUS"], array("user_id" => $arFilter["AUTHOR_ID"], "status_code" =>ToLower($StatusCode))),
                    "BLOG_URL"			=> $arResult["VARIABLES"]["blog"],
                ),
                $component 
            );
        ...
        <?$APPLICATION->IncludeComponent(
                ...
                "",
                Array(
                    "PATH_TO_CATEGORY_WITH_STATUS" => $arResult["PATH_TO_USER_IDEAS_STATUS"],
                    "PATH_TO_CATEGORY" => $arResult["PATH_TO_USER_IDEAS"],
