...
function getResultColumnDataType(&$viewColumnInfo)
{
					...
					if ($cur != $prev || $bLastValue)
					{
						...
						while ($n-- > 0)
						{
							...
							$strHtml .=
								...
								"\t\t\t\t".'<td'.$cellClassAttr.'><div'.$margin.'>'.$groupValue.'</div></td>'.PHP_EOL;
							foreach ($arSubTotal as $k => $subValue)
							{
								...
								$strHtml .=
									"\t\t\t\t".'<td'.$cellClass.'>'.$finalSubValue.'</td>'.PHP_EOL;
							}
							$strHtml .=
								"\t\t\t".'</tr>'.PHP_EOL;
							$strHtml .= $strSubHtml;
			...
			if (count($total) > 0)
			{
				...
				if ($bShowTotal)
				{
					...
					foreach ($total as $k => $v)
					{
						...
						$strHtml .=
							"\t\t\t\t".'<td class="adm-list-table-cell'.$cellClass.'">'.$finalTotalValue.'</td>'.PHP_EOL;
					}
					$strHtml .=
						"\t\t\t".'</tr>'.PHP_EOL;
				}
...
<?php
	...
	function prepareChartData(&$arResult)
	{
		// check
		...
		$chartTypeIds = array();
		foreach ($arResult['chartTypes'] as $chartTypeInfo) $chartTypeIds[] = $chartTypeIds[] = $chartTypeInfo['id'];
		...
		$yColumnsHumanTitles = array();
		$columnsTypes = array();
		$columnsTypes[0] = $xColumnDataType;
		for ($i = 0; $i < $yColumnsCount; $i++)
		{
			$yColumnIndex = $chartSettings['y_columns'][$i];
			$yColumnsResultNames[] = $arResult['viewColumns'][$yColumnIndex]['resultName'];
			$yColumnsHumanTitles[] = $arResult['viewColumns'][$yColumnIndex]['humanTitle'];
		...
		return array('requestData' => $requestData, 'columnsNames' => $yColumnsHumanTitles);
	}
							...
							if (response.legendInfo)
							{
								...
								for (var i in response.legendInfo)
								{
									...
									if (legendNewRow)
									{
										...
										legendStick = BX.findChild(legendNewRow, {class: 'report-chart-legend-stick'});
										legendLabel = BX.findChild(legendNewRow, {class: 'report-chart-legend-label'});
										...
										else
										{
											...
											legendLabel.innerHTML = columnsNames[i];
										}
