                                ...
                                <div class="blog-idea-body">
                                    <div class="idea-owner">
                                        <div class="bx-idea-condition-description status-color-<?=ToLower($arStatusList[$arResult["POST_PROPERTIES"]["DATA"]["UF_STATUS"]["VALUE"]]["XML_ID"]);?>">
                                            <div <?if($arResult["IDEA_MODERATOR"]):?>class="status-action idea-action-cursor" onclick="JSPublicIdea.ShowStatusDialog(this, '<?=$arResult["Post"]["ID"]?>');" id="status-<?=$arResult["Post"]["ID"]?>"<?endif;?>><?=GetMessage("IDEA_STATUS_".ToUpper($arStatusList[$arResult["POST_PROPERTIES"]["DATA"]["UF_STATUS"]["VALUE"]]["XML_ID"]))?></div>
                                        </div>
                                        ...
                                        <div class="idea-post-meta">
                                            <div class="idea-post-meta-util">
                                                <?if(strLen($arResult["urlToHide"])>0):?>
                                                    <a href="javascript:if(confirm('<?=GetMessage("BLOG_MES_HIDE_POST_CONFIRM")?>')) window.location='<?=$arResult["urlToHide"]."&".bitrix_sessid_get()?>'"><span><?=GetMessage("BLOG_MES_HIDE")?></span></a>
                                                <?elseif(strLen($arResult["urlToShow"])>0):?>
                                                    <a href="javascript:if(confirm('<?=GetMessage("IDEA_MES_SHOW_POST_CONFIRM")?>')) window.location='<?=$arResult["urlToShow"]."&".bitrix_sessid_get()?>'"><span><?=GetMessage("IDEA_MES_SHOW")?></span></a>
                                                <?endif;?>
                                                ...
                                                <?if(strLen($arResult["urlToDelete"])>0):?>
                                                    <a href="javascript:if(confirm('<?=GetMessage("BLOG_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$arResult["urlToDelete"]."&".bitrix_sessid_get()?>'"><span><?=GetMessage("BLOG_BLOG_BLOG_DELETE")?></span></a>
                                                <?endif;?>
                                            </div>
    ...
    if($arOfficial!==false):
            $cntOfficial = $APPLICATION->IncludeComponent(
                    ...
                    Array(
                                    ...
                                    "SEO_USER"			=> $arParams["EXT"][1]["SEO_USER"],
