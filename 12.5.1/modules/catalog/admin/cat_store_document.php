...
if($REQUEST_METHOD == "GET" && $ID > 0 && $action == 'conduct'  && check_bitrix_sessid())
{
	$DB->StartTransaction();
	...
	if($ex = $APPLICATION->GetException())
	{
		...
		if(!empty($result))
		{
			$arProduct = getProductInfo(array("ELEMENT_ID" => $result["PRODUCT"]));
			$strError = str_replace("#STORE#", "\"".getStoreName($result["STORE"])."\"", $strError);
			$strError = str_replace("#PRODUCT#", "\"".$arProduct["NAME"]."\"", $strError);
			$strError = str_replace("#BARCODE#", "\"".$result["BARCODE"]."\"", $strError);
		}
...
if($REQUEST_METHOD == "POST" && check_bitrix_sessid() && isset($_POST["DOCUMENT_TYPE"]))
{
	$docType = ($_POST["DOCUMENT_TYPE"]);
	$contractorId = intval($CONTRACTOR_ID);
	$currency = '';
	$result = array();
	$docId = 0;

	if($CAT_CURRENCY_STORE)
		$currency = $CAT_CURRENCY_STORE;
	...
	if(strlen($CAT_DOC_TOTAL_SAVE) > 0)
		$arGeneral["TOTAL"] = doubleval($CAT_DOC_TOTAL_SAVE);
	if($ID > 0)
	{
	...
	if(isset($_POST["PRODUCT"]) && is_array($_POST["PRODUCT"]) && $docId)
	{
		...
		foreach($_POST["PRODUCT"] as $key => $val)
		{
			...
			{
				foreach($val["BAR_CODE"] as $key => $barCode)
				{
					if(!empty($barCode))
		...
		if($save_and_conduct)
		{
			LocalRedirect("/bitrix/admin/cat_store_document.php?lang=".LANG."&ID=".$docId."&action=conduct&sessid=".$sessid."&".GetFilterParams("filter_", false));
		}
	}
	if($docId)
		LocalRedirect("/bitrix/admin/cat_store_document_list.php?lang=".LANG."&".GetFilterParams("filter_", false));
...
if(strlen($docType) > 0 && isset(CCatalogDocs::$types[$docType]))
{
	...
	$requiredFields = $documentClass::getFields();
}
else
	LocalRedirect("/bitrix/admin/cat_store_document_list.php?lang=".LANG."&".GetFilterParams("filter_", false));
...
$APPLICATION->SetTitle(GetMessage("CAT_DOC_NEW"));
...
$tabControl->Begin();
?>
	<form method="POST" action="/bitrix/admin/cat_store_document.php?lang=ru" enctype="multipart/form-data" id="form_catalog_document_form" name="form_catalog_document_form"><div class="adm-detail-block" id="form_order_buyers_layout" style="min-width: 165px;">
	...
	<tr class="adm-detail-required-field" id="company-name-tr" >
		...
		<td>
			<select size="1" id="SITE_ID" name="SITE_ID" <?=($bReadOnly) ? " disabled" : ""?>/>
	...
	<?if(is_set($requiredFields, "CURRENCY")):?>
		<tr>
			...
			<td><? echo CCurrency::SelectBox("CAT_CURRENCY_STORE", $str_CURRENCY, "", true, "", "id='CAT_CURRENCY_STORE'".$isDisabled);?></td>
		</tr>
	<?endif;?>
		...
		<tr>
			...
			<td>
				<?if(count($arContractors) > 0 && is_array($arContractors)):?>
					<select style="max-width:300px" size="1" name="CONTRACTOR_ID" <?=($bReadOnly) ? " disabled" : ""?>>
	...
	<?if(!$bReadOnly):?>
		<?if(is_set($requiredFields, "STORE_FROM")):?>
			<tr>
				...
				<td>
					<select size="1" style="max-width:300px" name="CAT_DOC_STORE_FROM" id="CAT_DOC_STORE_FROM" <?=($bReadOnly) ? " disabled" : ""?>>
						<?
						foreach($arStores as $key => $val)
						{
							$store = ($val["TITLE"] != '') ? htmlspecialcharsbx($val["TITLE"])." (".htmlspecialcharsbx($val["ADDRESS"]).")" : htmlspecialcharsbx($val["ADDRESS"]);
		...
		<?if(is_set($requiredFields, "STORE_TO")):?>
			<tr>
				...
				<td>
					<select size="1" style="max-width:300px" name="CAT_DOC_STORE_TO" id="CAT_DOC_STORE_TO" <?=($bReadOnly) ? " disabled" : ""?>>
						<?
						foreach($arStores as $key => $val)
						{
							$store = ($val["TITLE"] != '') ? htmlspecialcharsbx($val["TITLE"])." (".htmlspecialcharsbx($val["ADDRESS"]).")" : htmlspecialcharsbx($val["ADDRESS"]);
		...
		<td valign="top" align="left" colspan="2">
			...
			<div style="float:right">
				<script>
					...
					{
						var formBarcodes;
						formBarcodes = BX.PopupWindowManager.create("catalog-popup-barcodes-"+id, BX("product_name_" + id), {
							offsetTop : -100,
							offsetLeft : 200,
							autoHide : false,
							closeByEsc : true,
...
//    oCellBalance.setAttribute('class','balance_count');
		...
		<?if(!$bReadOnly):?>
		oCellQuantity.innerHTML = "<div align='center'><input maxlength=\"7\" onChange=\"fRecalProduct(" + ID + ", '', 'N', 'N');\" type=\"text\" name=\"PRODUCT[" + ID + "][QUANTITY]\" id=\"PRODUCT[" + ID + "][QUANTITY]\" value=\"" + quantity + "\" size=\"4\"></div><span class=\"warning_balance\" id=\"warning_balance_"+ID+"\"></span>";
		<?else:?>
		oCellQuantity.innerHTML = "<div align='center' id=\"PRODUCT[" + ID + "][QUANTITY_SPAN]\">" + quantity + "</div><span class=\"warning_balance\" id=\"warning_balance_"+ID+"\"></span>";
		<?endif;?>
		<?if(is_set($requiredFields, "NET_PRICE")):?>
		var priceColumn = "";
		...
		<?endif;?>
		<?if(is_set($requiredFields, "STORE_TO")):?>
		oCellStoreTo.innerHTML = "<input type='hidden' name=\"PRODUCT[" + ID + "][CAT_DOC_STORE_TO]\" value="+parentStoreTo+"><div align='center' id=\"DIV_STORE_TO"+ID+"\">" + "<select style=\"max-width:300px\" <?if($bReadOnly) echo " disabled";?> size='1' id=\"PRODUCT[" + ID + "][CAT_DOC_STORE_TO]\" name=\"PRODUCT[" + ID + "][CAT_DOC_STORE_TO]\"><?foreach($arStores as $key => $val)
		...
		if(isMultiBarcode == 'Y')
		{
			...
			oCellBarcode.innerHTML = "<div align='center'><a href=\"#\" onClick=\"enterBarcodes("+ID+", "+quan+");\" class=\"adm-btn\"><?=GetMessage('CAT_DOC_BARCODES_'.$action);?></a></div>";
		}
	...
	function fRecalProduct(id, type, recommendet, recalcAll)
	{
		...
		if(BX("PRODUCT["+id+"][PRICE]") && BX("PRODUCT["+id+"][QUANTITY]"))
		{
			...
			BX('DIV_SUMMA_' + id).innerHTML = "<div align='center'><span id='CAT_DOC_SUMM_"+id+"'>" + showPrice + "</span></div>";
		}
		...
		for(var i=1; i<=counter; i++)
		{
			...
			{
				totalSum = totalSum + Number(BX('CAT_DOC_SUMM_' + i).innerHTML);
			}
		}
		if(isNaN(totalSum))
			totalSum = 0;
		BX('CAT_DOC_TOTAL').innerHTML = " "+totalSum;
