...
if ($USER->CanDoOperation('catalog_read') || $USER->CanDoOperation('catalog_price'))
{
	...
	$bUseStoreControl = (COption::GetOptionString('catalog','default_use_store_control','N') == "Y") ? true : false;
	?>
<style type="text/css">
...
<tr class="heading">
	...
	?><script type="text/javascript">
			...
			function checkBarCode()
			{
				...
				{
					BX('tr_CAT_BARCODE').disabled = true;
					BX('tr_CAT_BARCODE').style.display = 'none';
				}
				else
				{
					BX('tr_CAT_BARCODE').disabled = false;
					BX('tr_CAT_BARCODE').style.display = 'table-row';
				}
			}
		</script>
	</td>
	...
	<?if($USER->CanDoOperation('catalog_purchas_info')):?>
		...
		<tr>
			...
			<td>
				...
				<? $isDisabled = ''; if($bUseStoreControl) $isDisabled = " disabled"; echo CCurrency::SelectBox("CAT_PURCHASING_CURRENCY", "", "", true, "", "id='CAT_PURCHASING_CURRENCY' $isDisabled");?></td>
			<script type="text/javascript">
				setSelectByValue("CAT_PURCHASING_CURRENCY", "<?=$arBaseProduct['PURCHASING_CURRENCY']?>");
			</script>
		</tr>
	<?endif;?>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="catalog_properties_table">
			...
			<tr id="CAT_BASE_QUANTITY_RESERV">
				...
				<td width="60%">
					...
					?>
					<input type="text" id="CAT_BASE_QUANTITY_RESERVED" name="CAT_BASE_QUANTITY_RESERVED" disabled readonly value="<?echo htmlspecialcharsbx($str_CAT_BASE_QUANTITY_RESERVED) ?>" size="30">
	...
	if(CBXFeatures::IsFeatureEnabled('CatMultiStore'))
	{
		...
		while ($arProp = $rsProps->GetNext())
		{
			...
			<tr>
				...
				<td style="text-align:center;"><input type="text" id="AR_AMOUNT_<? echo $arProp['ID']; ?>" name="AR_AMOUNT[<?=$arProp['ID']?>]" size="12" value="<? echo htmlspecialcharsbx($amount); ?>" <? echo ((!$bStore || $bUseStoreControl) ? 'disabled readonly ' : ''); ?>/></td><?
				if ($bStore)
				{
	...
	if($bUseStoreControl)
	{
		...
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="catalog_barcode_table">
			...
			<tr>
				...
				<td width="60%">
					<input type="checkbox" name="CAT_BARCODE_MULTIPLY" id="CAT_BARCODE_MULTIPLY" value="Y" onclick="checkBarCode()" <?=$bUseMultiplyBarcode ? 'checked="checked"' : ''?> <? echo (($bReadOnly) ? ' disabled readonly' : ''); ?>/>
				</td>
			</tr>

			<tr id="tr_CAT_BARCODE"<? echo (($bUseMultiplyBarcode) ? ' style="display: none;"' : ''); ?>>
				<td><?echo GetMessage("C2IT_BAR_CODE")?>:</td>
				<td><input type="text" name="CAT_BARCODE" id="CAT_BARCODE" size="30" value="<?=$barcode?>" <? echo (($bReadOnly) ? ' disabled' : ''); ?>/>
			</tr>
