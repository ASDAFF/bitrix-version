...
if($countersupdate == "Y" && $ID > 0)
{
	if(CControllerMember::UpdateCounters($ID))
	{
		...
		?>
		<script>
			if(parent.document.getElementById("COUNTERS_UPDATED"))
				parent.document.getElementById("COUNTERS_UPDATED").innerHTML = '<?echo CUtil::JSEscape($arMember["COUNTERS_UPDATED"])?>';
		<?if($arGroup["CHECK_COUNTER_FREE_SPACE"] == "Y"):?>
			if(parent.document.getElementById("COUNTER_FREE_SPACE"))
				parent.document.getElementById("COUNTER_FREE_SPACE").innerHTML = '<?echo CUtil::JSEscape($arMember["COUNTER_FREE_SPACE"])?>';
		<?endif?>
		<?if($arGroup["CHECK_COUNTER_SITES"] == "Y"):?>
			if(parent.document.getElementById("COUNTER_SITES"))
				parent.document.getElementById("COUNTER_SITES").innerHTML = '<?echo CUtil::JSEscape($arMember["COUNTER_SITES"])?>';
		<?endif?>
		<?if($arGroup["CHECK_COUNTER_USERS"] == "Y"):?>
			if(parent.document.getElementById("COUNTER_USERS"))
				parent.document.getElementById("COUNTER_USERS").innerHTML = '<?echo CUtil::JSEscape($arMember["COUNTER_USERS"])?>';
		<?endif?>
		<?if($arGroup["CHECK_COUNTER_LAST_AUTH"] == "Y"):?>
			if(parent.document.getElementById("COUNTER_LAST_AUTH"))
				parent.document.getElementById("COUNTER_LAST_AUTH").innerHTML = '<?echo CUtil::JSEscape($arMember["COUNTER_LAST_AUTH"])?>';
		<?endif?>
		<?
		$rsCounters = CControllerCounter::GetMemberValues($ID);
		while($arCounter = $rsCounters->Fetch())
		{?>
			if(parent.document.getElementById("COUNTER_<?echo $arCounter["ID"]?>"))
				parent.document.getElementById("COUNTER_<?echo $arCounter["ID"]?>").innerHTML = '<?echo CUtil::JSEscape($arCounter["DISPLAY_VALUE"])?>';
		<?
		}
		?></script><?
	}
	else
	{
		...
		?>
		<script>
		alert('<?=GetMessage("CTRLR_MEM_EDIT_COUNTER_ERR")?> <?=CUtil::JSEscape($err);?>');
		</script>
		<?
	}
	die();
...
?>
	...
	<tr class="adm-detail-required-field">
		...
		?>
		<input type="text" name="URL" size="42" maxlength="255" value="<?=$str_URL?>"></td>
	</tr>
	<tr class="adm-detail-required-field">
...
{
	countersupdate.location='/bitrix/admin/controller_member_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$ID?>&countersupdate=Y';
}
</script>
	<tr class="adm-detail-valign-top">
		...
		<IFRAME style="width:0px; height:0px; border:none;" src="javascript:void(0)" id="countersupdate" name="countersupdate"></IFRAME>
		</td>
	</tr>
	<?
	$mb = CControllerGroup::GetByID($str_CONTROLLER_GROUP_ID);
