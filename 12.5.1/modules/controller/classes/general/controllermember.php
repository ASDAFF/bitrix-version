...
{
	/******************************* Public methods ************************************/
	function CheckMember($member_id, $member_url = false)
	{
		if($member_url === false)
		{
	...
	function RunCommandRedirect($member_id, $command, $arParameters = Array(), $log = true)
	{
	...
	function RunCommandWithLog($member_id, $command, $arParameters = Array(), $task_id=false, $operation = 'run')
	{
		$arControllerLog = Array(
				'NAME'=>'REMOTE_COMMAND',
				'CONTROLLER_MEMBER_ID'=>$member_id,
	...
	function RunCommand($member_id, $command, $arParameters = Array(), $task_id=false, $operation = 'run')
	{
		global $DB;
	...
	function SendFileWithLog($member_id, $file)
	{
		$arControllerLog = Array(
				'NAME'=>'SEND_FILE',
				'CONTROLLER_MEMBER_ID'=>$member_id,
	...
	function RemoveGroupSettings($member_id)
	{
		return true;
	}

	function addHistoryNote($CONTROLLER_MEMBER_ID, $strNote)
	{
		global $DB, $USER;
	...
	function logChanges($CONTROLLER_MEMBER_ID, $arFieldsOld, $arFieldsNew, $strNote)
	{
		global $DB, $USER;
	...
	function RegisterMemberByPassword($ar_member, $admin_login, $admin_password, $controller_url = false)
	{
	...
	function CheckUserAuth($member_id, $login, $password, $arRemGroups = false)
	{
		global $APPLICATION;
	...
	function RegisterMemberByPHP($ar_member)
	{
		$controller_url = COption::GetOptionString("controller", "controller_url", ($GLOBALS["APPLICATION"]->IsHTTPS()?"https://":"http://").$_SERVER['HTTP_HOST']);
	...
	function AddCommand($member_guid, $command, $arAddParams = Array(), $task_id = false)
	{
		global $DB;
