...
class CAllControllerCounter
{
	...
	function UpdateGroups($ID, $arGroups)
	{
		global $DB;
		$ID = intval($ID);
	...
	function SetGroupCounters($CONTROLLER_GROUP_ID, $arCounters)
	{
		global $DB;
	...
	function GetMemberCounters($CONTROLLER_MEMBER_ID)
	{
		global $DB;
	...
	function UpdateMemberValues($CONTROLLER_MEMBER_ID, $arValues)
	{
		global $DB;
		$CONTROLLER_MEMBER_ID = intval($CONTROLLER_MEMBER_ID);
