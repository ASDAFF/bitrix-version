...
?>
	...
	<tr>
		<td><?=GetMessage("AD_SHOW_INTERVAL").":"?></td>
		<td><?
		echo CalendarPeriod("DATE_SHOW_FROM", htmlspecialcharsbx($str_DATE_SHOW_FROM), "DATE_SHOW_TO", htmlspecialcharsbx($str_DATE_SHOW_TO), "form1", "N");?></td>
	</tr>
	<?endif;?>
...
<?$tabControl->BeginNextTab();?>
		...
		<td width="60%"><?
			if ($isEditMode) :
				...
				while(list($sid, $arrS) = each($arrSites)):
					...
					/*<?=$disabled?>*/
					?>
					<input type="checkbox" name="arrSITE[]" value="<?=htmlspecialcharsbx($sid)?>" id="site_<?=htmlspecialcharsbx($sid)?>" <?=$checked?>>
					<?echo '[<a href="/bitrix/admin/site_edit.php?LID='.urlencode($sid).'&lang='.LANGUAGE_ID.'" title="'.GetMessage("AD_SITE_ALT").'">'.htmlspecialcharsex($sid).'</a>]&nbsp;<label for="site_'.htmlspecialcharsbx($sid).'">'.htmlspecialcharsex($arrS["NAME"])?></label>
					<br>
					<?
				endwhile;
	...
	<tr valign="top">
		...
		{
			?><input <?if ($ID > 0 && in_array($arType["SID"], $arrTYPE)) echo "checked"?> type="checkbox" name="arrTYPE[]" value="<?=htmlspecialcharsbx($arType["SID"])?>" id="arType_<?=$i?>"><?echo "[<a href='/bitrix/admin/adv_type_edit.php?lang=".LANGUAGE_ID."&SID=".$arType["SID"]."&action=view' title='".GetMessage("AD_TYPE_ALT")."'>".$arType["SID"]."</a>] "."<label for='".$arType."_".$i."'>".$arType["NAME"]?></label><br>
			<?
			$i++;
		}
