...
if(empty($arAjaxResult))
{
    switch ($_REQUEST["ACTION"])
    {
        case "GET_LIFE_SEARCH":
            if (!defined('BX_UTF') && strlen($arResult["LIFE_SEARCH_QUERY"])>0)
                $arResult["LIFE_SEARCH_QUERY"] = $APPLICATION->ConvertCharset($arResult["LIFE_SEARCH_QUERY"], "UTF-8", SITE_CHARSET);
        ...
        case "SUBSCRIBE":
            ...
            if($USER->IsAuthorized() && $IdeaId>0)
            {
                $bNotify = CIdeaManagment::getInstance()->Notification()->getEmailNotify()->Add($IdeaId);
                if($bNotify)
                {
