...
<?
$CategoryCode = ToUpper($arResult["VARIABLES"]["category_2"]);
$CategoryCodeBase = ToUpper($arResult["VARIABLES"]["category_1"]);
$StatusCode = ToUpper($arResult["VARIABLES"]["status_code"]);

$arFilter = array(
    "IDEA_STATUS" => $StatusCode,
    "IDEA_PARENT_CATEGORY_CODE" => $CategoryCode,
);
...
<div class="idea-managment-content">
    ...
    <?$this->SetViewTarget("idea_filter", 100)?>
        <?if($arParams["DISABLE_RSS"] != "Y"):?>
            <?$APPLICATION->IncludeComponent(
                ...
                Array(
                    ...
                    "PATH_TO_RSS"		=> CComponentEngine::MakePathFromTemplate($arResult["PATH_TO_RSS_CATEGORY_STATUS"], array("category" => ToLower($CategoryCode), "status_code" => ToLower($StatusCode))),
                    "BLOG_URL"			=> $arResult["VARIABLES"]["blog"],
                ),
                $component 
            );
        ...
        <?$APPLICATION->IncludeComponent(
                ...
                "",
                Array(
                    "PATH_TO_CATEGORY_WITH_STATUS" => $arResult["PATH_TO_CATEGORY_2_STATUS"],
                    "PATH_TO_CATEGORY" => $arResult["PATH_TO_CATEGORY_2"],
                    "SELECTED_STATUS" => $StatusCode,
                    "CATEGORIES" => array("CATEGORY_1" => $CategoryCodeBase, "CATEGORY_2" => $CategoryCode),
                    "SET_NAV_CHAIN" => $arParams["SET_NAV_CHAIN"],
                ),
                $component
        );
...
if($arParams["SET_NAV_CHAIN"] == "Y")
{
    ...
    if(isset($arCategoryList[$CategoryCodeBase])) 
        $APPLICATION->AddChainItem($arCategoryList[$CategoryCodeBase]["NAME"], str_replace("#category_1#", $arResult["VARIABLES"]["category_1"], $arResult["PATH_TO_CATEGORY_1"]));
    if(isset($arCategoryList[$CategoryCode])) 
        $APPLICATION->AddChainItem($arCategoryList[$CategoryCode]["NAME"], str_replace(array("#category_1#","#category_2#"), array($arResult["VARIABLES"]["category_1"], $arResult["VARIABLES"]["category_2"]), $arResult["PATH_TO_CATEGORY_2"]));
}
//Set Title
if($arResult["SET_TITLE"] == "Y")
    $APPLICATION->SetTitle(GetMessage("IDEA_CATEGORY_2_STATUS_PAGE_TITLE").': '.$arCategoryList[$CategoryCode]["NAME"]);
?>