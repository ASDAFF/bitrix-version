	...
	foreach($arPersonTypeList as $arPersonType)
	{
		...
		<tr class="adm-detail-required-field">
			...
			<td width="60%">
				...
				{
					?>
					arActionPaths[<?= $i ?>] = '<?= $val["PATH"] ?>';
					arActionDescrs[<?= $i ?>] = '<?= CUtil::JSEscape($val["DESCRIPTION"]) ?>';
					<?
				}
				$ij = 0;
				...
				if ($bInitVars)
				{
					...
					foreach ($arActFields as $val)
					{
						...
						if (strlen($typeTmp) <= 0)
							$valueTmp = ${"VALUE2_".$val."_".$arPersonType["ID"]};
						?>
						var param_<?= $val ?>_type_<?= $arPersonType["ID"] ?> = '<?= CUtil::JSEscape(${"TYPE_".$val."_".$arPersonType["ID"]}) ?>';
