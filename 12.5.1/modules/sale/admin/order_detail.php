...
if (strlen($filter_marked)>0) $arFilterHistory["MARKED"] = trim($filter_marked);
if (strlen($filter_deducted)>0) $arFilterHistory["DEDUCTED"] = trim($filter_deducted);
...
$histdHeader = array(
	...
	array("id"=>"DEDUCTED", "content"=>$arFieldsAll["DEDUCTED"], "sort"=>"DEDUCTED", "default"=>true),
	array("id"=>"MARKED", "content"=>$arFieldsAll["MARKED"], "sort"=>"MARKED", "default"=>true),
...
while ($arHistory = $dbHistory->Fetch())
{
	...
	$mark = "";
	if ($arHistory["MARKED"] == "Y")
		$mark = GetMessage("SOD_HIST_YES");
	elseif ($arHistory["MARKED"] == "N")
		$mark = GetMessage("SOD_HIST_NO");
	$row->AddField("MARKED", $mark);

	$deduct = "";
	if ($arHistory["DEDUCTED"] == "Y")
		$deduct = GetMessage("SOD_HIST_YES");
	elseif ($arHistory["DEDUCTED"] == "N")
		$deduct = GetMessage("SOD_HIST_NO");
	$row->AddField("DEDUCTED", $deduct);
	...
	foreach ($arHistory as $key => $val)
	{
		if (strlen($val) > 0 && !in_array($key, $arDeleteFields))
		{
			...
			elseif ($key == "EMP_PAYED_ID")
				$val = GetFormatedUserName($val);
			elseif ($key == "REASON_CANCELED")
				$val = htmlspecialcharsbx($val);
			elseif ($key == "REASON_UNDO_DEDUCTED")
				$val = htmlspecialcharsbx($val);
			...
			$tabControl->AddSection("order_payment", GetMessage("P_ORDER_PAYMENT"));
				...
				$tabControl->BeginCustomField("ORDER_MARKED", GetMessage("SOD_MARKED_Y"));
					...
					<tr id="btn_mark_cancel_button" style="display:<?=($arOrder["MARKED"] != "N") ? 'table-row' : 'none'?>">
						<td width="40%">&nbsp;</td>
						<td>
							<?if($bUserCanMarkOrder):?><a href="javascript:void(0);" onClick="fCancelMarkOrder();" class="adm-btn-wrap"><span class="adm-btn"><?=GetMessage('SOD_MARK_N');?></span></a><?endif;?>
						</td>
					</tr>
					<tr>
						<td valign="top">
					...
					<td colspan="2" valign="top">
						...
						if(!empty($arBasketId))
						{
							...
							foreach ($arBasketValue as $arBasket)
							{
								...
								$ORDER_TOTAL_WEIGHT += FloatVal($arBasket["WEIGHT"] * $arBasket["QUANTITY"]);
							?>
							<tr>
								<td class="photo">
									<?
...
<div id="tr-sourse" style="display:none;">
	...
	<tr>
		<td><?=GetMessage('SOD_HIST_DEDUCTED')?>:</td>
		<td>
	...
	<tr>
		<td><?=GetMessage('SOD_HIST_MARKED')?>:</td>
		<td>
