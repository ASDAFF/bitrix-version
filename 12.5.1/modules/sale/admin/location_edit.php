	...
	<tr class="heading">
		<td colspan="2"><?echo GetMessage("SALE_REGION")?>
			<script>
				...
				{
					var regonVal = document.getElementById('REGION_ID').value;

					if (regonVal == "" || regonVal > 0)
					{
						enabled = false;
					}
					else
	...
	<tr class="adm-detail-required-field">
		...
		<td>
			<select name="REGION_ID" id="REGION_ID" OnChange="ResetRegion()">
				...
				{
					?><option value="<?echo $arRegionList["ID"] ?>"<?if (IntVal($arRegionList["ID"])==IntVal($str_REGION_ID)) echo " selected";?>><?echo htmlspecialcharsEx($arRegionList["NAME_ORIG"]) ?></option><?
				}
				?>
			</select>
		</td>
