...
// echo "<pre>"; print_r($arResult["SKU_ELEMENTS"]); echo "</pre>";
			foreach ($arResult["SKU_ELEMENTS"] as $val)
			{
				...
				if ($addDefault == "Y" || ($val["CAN_BUY"] == "Y" && $addDefault == "N"))
				{
					...
					$arParams = array();
					$arParams['id'] = $val["ID"];
					$arParams['name'] =  str_replace($aSearch, $aReplace, htmlspecialcharsback($val["NAME"]));
					...
					$arParams['barcodeMulti'] = CUtil::JSEscape($arCatalogProduct["BARCODE_MULTI"]);
					$arParams['stores'] = CUtil::JSEscape(CUtil::PhpToJSObject($arStores));
