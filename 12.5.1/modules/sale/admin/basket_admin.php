...
if (isset($find_universal) && strlen($find_universal) > 0)
	$arFilter["%NAME_SEARCH"] = trim($find_universal);
...
while ($arBasket = $dbResultList->Fetch())
{
	...
	while($arB = $dbB->Fetch())
	{
		...
		$basket .= " (".$arB["QUANTITY"]." ".GetMessage("SB_SHT").") - "."<nobr>".SaleFormatCurrency($arB["PRICE"], $arB["CURRENCY"])."</nobr><br>";
...
?>
	<tr>
		...
		<td>
			<input type="text" name="find_universal" value="<?echo htmlspecialcharsbx($find_universal)?>" size="40">
		</td>
	</tr>
	<tr>
