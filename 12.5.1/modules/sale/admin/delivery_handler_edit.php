...
<?
	switch ($arConfig["TYPE"])
	{
		...
		case "DROPDOWN":
		?>
			<select name="HANDLER[CONFIG][<?=htmlspecialcharsbx($config_id)?>]">
		<?
		...
		case "MULTISELECT":
		?>
			<select name="HANDLER[CONFIG][<?=htmlspecialcharsbx($config_id)?>]" multiple="multiple">
		<?

			foreach ($arConfig["VALUES"] as $value => $title):
		?>
				<option value="<?=htmlspecialcharsEx($value)?>"<?=(in_array($value, $arConfigValues[$config_id]) ? " selected=\"selected\"" : "")?>><?=htmlspecialcharsEx($title)?></option>
		<?
			endforeach;
		?>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table">
	<tr>
		...
		<td width="60%">
			...
			<?
			$arPaySystemId = array();
			$dbRes = CSaleDelivery::GetDelivery2PaySystem(array("DELIVERY_ID" => $SID));
			while ($arRes = $dbRes->Fetch())
				$arPaySystemId[] = $arRes["PAYSYSTEM_ID"];
			?>
				<option value="" <?=(count($arPaySystemId) <= 0)?"selected":""?>><?=GetMessage("SDEN_PAYSYSTEM_ALL");?></option>
			<?
			...
			?>
				<option value="<?=intval($arPayType["ID"]);?>" <?=(in_array($arPayType["ID"], $arPaySystemId)?"selected":"")?>><?=htmlspecialcharsbx($arPayType["NAME"])?></option>
			<?endwhile;?>
			</select>
		</td>
	</tr>
