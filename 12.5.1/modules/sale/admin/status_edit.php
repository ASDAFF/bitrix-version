	...
	if (strlen($strError)<=0)
	{
		...
		while ($arGroup = $dbGroups->Fetch())
		{
			...
			$arPerms[] = array(
					...
					"PERM_MARK" => ${"PM_MARK_".$arGroup["ID"]},
					"PERM_DELIVERY" => ${"PM_DELIVERY_".$arGroup["ID"]},
					"PERM_DEDUCTION" => ${"PM_DEDUCTION_".$arGroup["ID"]},
	...
	<tr>
		<td colspan="2" align="center">
			...
			<tr class="heading">
				...
				<td><?= GetMessage("SSEN_PERM_MARK") ?></td>
				<td><?= GetMessage("SSEN_PERM_DELIVRY") ?></td>
				<td><?= GetMessage("SSEN_PERM_DEDUCTION") ?></td>
			...
			while ($arPM = $dbPermsMatrix->Fetch())
			{
				$arPermsMatrix[$arPM["GROUP_ID"]] = array(
					...
					"PERM_MARK" => $arPM["PERM_MARK"],
					"PERM_DELIVERY" => $arPM["PERM_DELIVERY"],
					"PERM_DEDUCTION" => $arPM["PERM_DEDUCTION"],
			...
			while ($arGroup = $dbGroups->Fetch())
			{
				...
				if (isset($arPermsMatrix[$arGroup["ID"]]))
				{
					...
					$str_PM_MARK = $arPermsMatrix[$arGroup["ID"]]["PERM_MARK"];
					$str_PM_DELIVERY = $arPermsMatrix[$arGroup["ID"]]["PERM_DELIVERY"];
					$str_PM_DEDUCTION = $arPermsMatrix[$arGroup["ID"]]["PERM_DEDUCTION"];
				...
				if ($bInitVars)
				{
					...
					$str_PM_DEDUCTION = ${"PM_DEDUCTION_".$arGroup["ID"]};
				...
				<tr>
					...
					<td align="center" style="text-align:center;"><input type="checkbox"<?if ($str_PM_DEDUCTION == "Y") echo "checked";?> name="PM_DEDUCTION_<?= $arGroup["ID"] ?>" OnClick="PMOnClick(<?= $arGroup["ID"] ?>)" value="Y"></td>
