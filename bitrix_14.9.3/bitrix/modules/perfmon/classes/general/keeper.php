...
class CAllPerfomanceKeeper
{
	...
	function writeToDatabase()
	{
		...
		if ($cache_log)
		{
			...
			if ($large_cache_log)
				self::removeCaches($large_cache_size, $arCacheDebug);
	...
	function removeCaches($large_cache_size, &$arCacheDebug)
	{
		global $APPLICATION;
		...
		if (is_array($APPLICATION->arIncludeDebug))
		{
			foreach($APPLICATION->arIncludeDebug as $i => $ar)
			{
				if (isset($ar["CACHE"]) && is_array($ar["CACHE"]))
				{
					foreach($ar["CACHE"] as $N => $arCacheInfo)
					{
						...
						{
							unset($APPLICATION->arIncludeDebug[$i]["CACHE"][$N]);
						}
					}
				}
			}
