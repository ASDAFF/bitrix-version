...
// ALLOW_UPLOAD_EXT = comma-separated list of allowed file extensions (ALLOW_UPLOAD='F')
		...
		if (this.init(this.fileInput)) // init fileInput
		{
			...
			this.uploadFileUrl = (!!params["uploadFileUrl"] ? params["uploadFileUrl"] : this.form.getAttribute("action"));
			...
			if (!!upload)
			{
				...
				for (id in stream.files)
				{
					if (stream.files.hasOwnProperty(id))
					{
						...
						if (!!item)
						{
							this.adjustProcess(stream.id, item, statuses.inprogress, 0);
						}
					}
				}
			}
			...
			if (!!upload)
			{
				...
				for (id in stream.files)
				{
					if (stream.files.hasOwnProperty(id))
					{
						...
						if (!!item)
						{
							this.adjustProcess(stream.id, item, statuses.inprogress, procent);
						}
					}
				}
			}
	...
	BX.UploaderSimple.prototype.onFileNeedsPreviewCallback = function(packageIndex, data)
	{
		...
		if (!!queue)
		{
			...
			while((item = queue.getFirst()) && !!item)
			{
				...
				if (checked === null)
					this.adjustProcess(null, item, statuses.error, {error : BX.message('UPLOADER_UPLOADING_ERROR')});
				else
					this.adjustProcess(null, item, statuses.error, {error : data["files"][item.id]["error"]});
			}
		}
	};
	...
	BX.UploaderStreams.prototype = {
		get : function(pack, post)
		{
			if (!this.stream && (this.stream = this.streams.getFirst()) && !!this.stream)
			{
				...
				if (!this.packages.hasItem(pack.packageIndex))
					this.packages.setItem(pack.packageIndex, post);
				this.stream.init(pack.packageIndex, this.packages.getItem(pack.packageIndex));
			}
			return this.stream;
		},
		release : function()
