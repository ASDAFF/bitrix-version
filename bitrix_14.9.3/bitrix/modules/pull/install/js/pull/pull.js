...
;(function(window){
						...
						if (data && data.ERROR == "")
						{
							if (message.id)
							{
								...
								if (!_channelStack[''+message.channel+message.id])
								{
									...
									BX.PULL.executeMessages(data.MESSAGE, message.time);
								}
							}
						}
						else
			...
			var _ajax = BX.ajax({
				...
				onsuccess: function(data)
				{
					...
					if (_pullMethod=='PULL' && typeof(data) == "object")
					{
						...
						{
							_sendAjaxTry = 0;
							BX.PULL.executeMessages(data.MESSAGE, (new Date()).toUTCString());
							if (_lsSupport)
	...
	{
		time = time === null? (new Date()).toUTCString(): time;
		...
		for (var i = 0; i < message.length; i++)
		{
			...
			message[i].params['SERVER_TIME'] = time;
	...
	BX.PULL.storageSet = function(params)
	{
		if (params.key == 'pus')
		{
			...
			BX.PULL.executeMessages(params.value.MESSAGE, params.value.TIME, false);
		}
