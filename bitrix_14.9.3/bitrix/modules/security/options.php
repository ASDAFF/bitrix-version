...
$arAllOptions = Array(
	...
	array("security_event_format", GetMessage("SEC_OPTIONS_EVENT_FORMAT"), array("text", 60)),
	array("security_event_userinfo_format", GetMessage("SEC_OPTIONS_EVENT_USERINFO_FORMAT"), array("text", 60)),
...
$tabControl->BeginNextTab();
	...
	<tr>
		<td colspan="2">
			<?=BeginNote();?>
			<?=GetMessage("SEC_OPTIONS_EVENT_MESSAGE_PLACEHOLDERS")?>:<br>
			<?foreach($availableMessagePlaceholders as $placeholder):?>
				<?=$placeholder?> - <?=getMessage("SEC_OPTIONS_EVENT_MESSAGE_PLACEHOLDER_".str_replace("#", "", $placeholder))?><br>
			<?endforeach?>
			<?=EndNote(); ?>
		</td>
	</tr>
	<tr>
		<td colspan="2">
			<?=BeginNote();?>
			<?=GetMessage("SEC_OPTIONS_EVENT_USERINFO_PLACEHOLDERS")?>:<br>
			<?foreach($availableUserInfoPlaceholders as $placeholder):?>
				<?=$placeholder?> - <?=getMessage("SEC_OPTIONS_EVENT_USERINFO_PLACEHOLDER_".str_replace("#", "", $placeholder))?><br>
			<?endforeach?>
			<?=EndNote(); ?>
		</td>
	</tr>
...
<?$tabControl->Buttons();?>
	...
	<?endif?>
	<input <?if(!$canWrite) echo "disabled" ?> type="submit" name="RestoreDefaults" title="<?echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" OnClick="confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>')" value="<?echo GetMessage("MAIN_RESTORE_DEFAULTS")?>">
	<?=bitrix_sessid_post();?>
<?$tabControl->End();?>
</form>
<?endif;?>