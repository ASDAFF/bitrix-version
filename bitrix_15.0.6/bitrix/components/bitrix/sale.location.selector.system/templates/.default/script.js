(function(){
	...
	if(typeof BX.systemLocationSelector == 'undefined'){

		BX.slsTree = function(opts, nf){

			this.parentConstruct(BX.slsTree, opts);
			...
			this.handleInitStack(nf, BX.slsTree, opts);
		};
		BX.extend(BX.slsTree, BX.ui.itemTree);

		// the following functions can be overrided with inheritance
		BX.merge(BX.slsTree.prototype, {

			// member of stack of initializers, must be defined even if do nothing
			init: function(){
				this.pushFuncStack('toggleRoot', BX.slsTree);
			},
			...
			refineResponce: function(responce){
				...
				for(var k in responce.ITEMS){
					...
					var isParent = parseInt(responce.ITEMS[k].CHILD_CNT) > 0;

					result.items.push({
						name: 				responce.ITEMS[k].NAME,
						id: 				responce.ITEMS[k].ID,
				...
				if(typeof responce.ETC.TOTAL_ITEM_COUNT != 'undefined')
					result.total = parseInt(responce.ETC.TOTAL_ITEM_COUNT);
		...
		BX.systemLocationSelector = function(opts, nf){

			this.parentConstruct(BX.systemLocationSelector, opts);
			...
			this.handleInitStack(nf, BX.systemLocationSelector, opts);
		}
		BX.extend(BX.systemLocationSelector, BX.ui.autoComplete);
		BX.merge(BX.systemLocationSelector.prototype, {
			...
			init: function(){
				...
				sc.selectedGroupsCntr = this.getControl('selected-group-counter');

				sc.inputPool = this.getControl('input-pool');

				sv.tree = new BX.slsTree({
					scope: ctx.getControl('selector-locations-tree'),
					source: so.source
				});

				this.pushFuncStack('buildUpDOM', BX.systemLocationSelector);
				this.pushFuncStack('bindEvents', BX.systemLocationSelector);
			},
			...
			refineRequest: function(request){
				...
				return {
					...
					BEHAVIOUR: BX.merge({SEARCH_BY_PRIMARY: '0'/*'1'*/}, BX.clone(this.opts.query.BEHAVIOUR)),
			...
			selectLinkItem: function(id, kind){
				var node = this.makeSelecteItemView(id, kind);
			...
			makeSelecteItemView: function(id, kind){
			...
			showSelectedNodePage: function(){
				...
				this.downloadPath(absentPath, BX.proxy(function(){
					...
					for(var i = 0; i < items.length; i++){
						...
						var node = this.makeSelecteItemView(sv.selected.nodes[items[i]].id, 'nodes');
			...
			showSelectedGroups: function(){
				...
				for(var i = 0; i < sv.selected.grp.length; i++){
					...
					var node = this.makeSelecteItemView(sv.selected.grp[i].id, 'grp');
				...
				BX.ajax({
					...
					onsuccess: function(result){
						...
						if(result.result){
							...
							BX.merge(sv.cache.path, result.data.PATH_NAMES);
