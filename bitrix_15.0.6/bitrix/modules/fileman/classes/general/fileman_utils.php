	...
	function Request($action, $site)
	{
		...
		switch($action)
		{
			...
			case "pack":
				...
				if (!$USER->CanDoFileOperation('fm_create_new_file', $docRootTo.$pack_to))
				{
					?>
					<script>
			...
			case "unpack":
				...
				if (!($USER->CanDoFileOperation('fm_create_new_file', $docRootTo.$_POST["packTo"]) ||
				$USER->CanDoFileOperation('fm_create_new_folder', $docRootTo.$_POST["packTo"])))
				{
					?>
					<script>
						window.BXFM_archivePermsError = true;
