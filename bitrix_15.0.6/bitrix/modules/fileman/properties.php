...
class CIBlockPropertyMapGoogle extends CIBlockPropertyMapInterface
{
	...
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		$MAP_ID = 'map_google_'.$arProperty['CODE'].'_'.$arProperty['ID'].'__n'.$GLOBALS['googleMapLastNumber'].'_';
...
<input type="text" style="display:none;" id="value_<?echo $MAP_ID;?>" name="<?=htmlspecialcharsbx($strHTMLControlName["VALUE"])?>" value="<?=htmlspecialcharsEx($value["VALUE"])?>" />
<script>
...
?>
BX.ready(setTimeout(BXWaitForMap_<?echo $MAP_ID?>, 1));
<?
endif;
?>
			...
			else
			{
				...
				$GLOBALS['APPLICATION']->IncludeComponent(
					...
					array(
						...
						'MAP_ID' => 'MAP_GOOGLE_VIEW_'.$arProperty['IBLOCK_ID'].'_'.$arProperty['ID'].'__n'.$GLOBALS['googleMapLastNumber'].'_',
						'DEV_MODE' => 'Y',
					),
					false, array('HIDE_ICONS' => 'Y')
				);
