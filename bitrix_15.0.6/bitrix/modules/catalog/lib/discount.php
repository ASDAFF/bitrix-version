	...
	public static function convertCurrency(&$discount, $currency)
	{
		...
		switch ($discount['VALUE_TYPE'])
		{
			...
			case self::VALUE_TYPE_SALE:
				$discount['VALUE'] = \CCurrencyRates::convertCurrency($discount['VALUE'], $discount['CURRENCY'], $currency);
				$discount['CURRENCY'] = $currency;
				break;
			case self::VALUE_TYPE_PERCENT:
				if ($discount['MAX_DISCOUNT'] > 0)
					$discount['MAX_DISCOUNT'] = \CCurrencyRates::convertCurrency($discount['MAX_DISCOUNT'], $discount['CURRENCY'], $currency);
				$discount['CURRENCY'] = $currency;
				break;
		}
	}
