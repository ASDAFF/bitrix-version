...
class CWD_Popup {
	...
	function JSAddLinkTo($arParams) {
		?>
		<input type="button" id="wd_func_opener_<?=$arParams['ID']?>" value="" style="display:none" />
		<script>
			if (window.WD_Popup_LinkTo==undefined) window.WD_Popup_LinkTo = [];
			var WD_Popup_JSON = <?=self::LinkToJSON(self::LinkEx($arParams));?>;
			window.WD_Popup_LinkTo["#wd_func_opener_<?=$arParams['ID']?><?if(trim($arParams['LINK_TO'])!=''):?>, <?=$arParams['LINK_TO']?><?endif?>"] = WD_Popup_JSON;
		</script>
		<?
	}
}
