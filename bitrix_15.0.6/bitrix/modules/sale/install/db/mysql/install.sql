...
(
	...
	primary key (ID),
	unique IX_B_SALE_LOC_CODE(CODE),
	index IX_B_SALE_LOC_MARGINS(LEFT_MARGIN, RIGHT_MARGIN),
	index IX_B_SALE_LOC_MARGINS_REV(RIGHT_MARGIN, LEFT_MARGIN),
	index IX_B_SALE_LOC_PARENT(PARENT_ID),
	index IX_B_SALE_LOC_DL(DEPTH_LEVEL),
	index IX_B_SALE_LOC_TYPE(TYPE_ID),
...
(
	...
	primary key (ID),
	index IX_B_SALE_LOC_NAME_NAME_U(NAME_UPPER),
	index IX_B_SALE_LOC_NAME_LI_LI(LOCATION_ID, LANGUAGE_ID)
);
...
(
	...
	primary key (ID),
	index IX_B_SALE_LOC_TYPE_NAME_TI_LI(TYPE_ID, LANGUAGE_ID)
);
...
(
	...
	SORT int not null default '100',
	primary key (ID),
	unique IX_B_SALE_LOC_GROUP_CODE(CODE)
);
